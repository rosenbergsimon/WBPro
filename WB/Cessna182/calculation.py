...
class Calculation:
    def __init__(self, data):
        ...
        if self.data['ident'] == "C-GMFB":
            self.fuel_arm = 46
            self.empty_weight = 1972.3
            self.empty_arm = 37.55
        ...
        self.pax2_moment = round(self.pax2 * 74, 2)
        self.bag1_moment = round(self.bag1 * 97, 2)
        self.bag2_moment = round(self.bag2 * 116, 2)
        self.bag3_moment = round(self.bag3 * 129, 2)
        self.empty_moment = round(self.empty_weight * self.empty_arm, 2)

        self.zero_fuel_weight = round(self.empty_weight + self.pax1 + self.pax2 + self.bag1 + self.bag2 + self.bag3, 2)
    ...
    def check_wb(self):
        ...
        if self.ramp_weight > 3110:
            ramp_weight_good = False
        if self.takeoff_weight > 3100:
            to_weight_good = False
        if self.landing_weight > 2950:
        ...
        # fuel reserves in gallons and hours calculated here using 15 GPH
        reserve_gallons = round((self.landing_weight - self.zero_fuel_weight) / 6, 1)
        reserve_hours = round(reserve_gallons / 15, 1)
    ...
    def check_in_limits(self, weight, arm):
        ...
        arm = round(arm, 2)
        if weight < 2250 and 33 <= arm <= 46:
            return 1
        if weight < 2250 and arm < 33:
            return 0
        if (2250 <= weight <= 3100) and 40.9 <= arm <= 46:
            return 1
        if arm > 46:
            return 2
        # the below is the top-left area on the graph, uses the bottom left as origin of the graph and diagonal line
        # has a slope of x
        if (2250 <= weight <= 3100) and arm < 40.9:
            a = weight - 2250
            b = arm - 33
            c = 107.59493671 * b
            if a <= c:
    ...
    def create_sheet(self):
        ...
        column1 = ["Front Seats", "Rear Seats", "Baggage Area A", "Baggage Area B", "Baggage Area C",
                   "Basic Empty Weight", "Zero Fuel Weight", "Fuel - Wing Tanks", "Ramp Weight", "Less Fuel for Taxi",
                   "Takeoff Weight", "Less Fuel for Flight", "Landing Weight"]
        column2 = [37, 74, 97, 116, 129, self.empty_arm, self.zero_fuel_arm, self.fuel_arm, self.ramp_arm,
                   self.fuel_arm, self.takeoff_arm, self.fuel_arm, self.landing_arm]
        column3 = [self.pax1, self.pax2, self.bag1, self.bag2, self.bag3, self.empty_weight, self.zero_fuel_weight,
                   self.fuel_load, self.ramp_weight, self.fuel_taxi, self.takeoff_weight, self.fuel_flight,
                   self.landing_weight]
        column4 = [self.pax1_moment, self.pax2_moment, self.bag1_moment, self.bag2_moment, self.bag3_moment,
                   self.empty_moment, self.zero_fuel_moment, self.fuel_load_moment, self.ramp_moment,
                   self.fuel_taxi_moment, self.takeoff_moment,self.fuel_flight_moment, self.landing_moment]
        ...
        zf_weight_cell = table.get_celld()[(7, 0)]
        zf_weight_cell.set_facecolor("dodgerblue")

        ramp_weight_cell = table.get_celld()[(9, 0)]
        ramp_weight_cell.set_facecolor("darkorange")

        to_weight_cell = table.get_celld()[(11, 0)]
        to_weight_cell.set_facecolor("gray")

        ldg_weight_cell = table.get_celld()[(13, 0)]
