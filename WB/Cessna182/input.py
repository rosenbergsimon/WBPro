...
class Input(tk.CTkFrame):
    def __init__(self, master, calculate, data):
        ...
        self.data = data
        self.label_font = tk.CTkFont(family="ebrima", size=12)
        ...
        # specific options to the 182s here
        self.ac_options = ["C-GMFB"]
        # sets a StringVar to represent value of the aircraft selected, and to call functions
        self.ac_option = tk.StringVar()  # This code will change the fuel max number on the fuel label
        self.ac_option.set("C-GMFB")  # when the box is clicked to new plane selection
        ...
        self.title_label.grid(column=0, columnspan=2, row=1, padx=20, pady=17)

        self.ac_label = tk.CTkLabel(self, text="Select an Airplane from the menu:", font=self.label_font,
                                    width=354, anchor="w")
        self.ac_label.grid(column=0, row=2, padx=20, pady=17)
        ...
        self.ac_box.grid(column=1, row=2, padx=20, pady=17)

        self.front_label = tk.CTkLabel(self, text="Enter the weight (lbs) in the front seats", font=self.label_font,
                                       width=354, anchor="w")
        self.front_label.grid(column=0, row=3, padx=20, pady=17)
        self.front_entry = tk.CTkEntry(self, width=100, font=self.label_font)
        self.front_entry.grid(column=1, row=3, padx=20, pady=17)

        self.back_label = tk.CTkLabel(self, text="Enter the weight (lbs) in the back seats", font=self.label_font,
                                      width=354, anchor="w")
        self.back_label.grid(column=0, row=4, padx=20, pady=17)
        self.back_entry = tk.CTkEntry(self, width=100, font=self.label_font)
        self.back_entry.grid(column=1, row=4, padx=20, pady=17)

        self.bag1_label = tk.CTkLabel(self, text="Enter the weight (lbs) in Baggage Area A (max 120)",
                                      width=354, font=self.label_font, anchor="w")
        self.bag1_label.grid(column=0, row=5, padx=20, pady=17)
        self.bag1_entry = tk.CTkEntry(self, width=100, font=self.label_font)
        self.bag1_entry.grid(column=1, row=5, padx=20, pady=17)

        self.bag2_label = tk.CTkLabel(self, text="Enter the weight (lbs) in Baggage Area B (max 80)",
                                      width=354, font=self.label_font, anchor="w")
        self.bag2_label.grid(column=0, row=6, padx=20, pady=17)
        self.bag2_entry = tk.CTkEntry(self, width=100, font=self.label_font)
        self.bag2_entry.grid(column=1, row=6, padx=20, pady=17)

        self.bag3_label = tk.CTkLabel(self, text="Enter the weight (lbs) in Baggage Area C (max 80)",
                                      width=354, font=self.label_font, anchor="w")
        self.bag3_label.grid(column=0, row=7, padx=20, pady=17)
        self.bag3_entry = tk.CTkEntry(self, width=100, font=self.label_font)
        self.bag3_entry.grid(column=1, row=7, padx=20, pady=17)

        self.bag_note = tk.CTkLabel(self, text="Note: Baggage area A, B, and C combined limit = 200lbs.",
                                    width=494, font=self.label_font, anchor="w")
        self.bag_note.grid(column=0, row=8, columnspan=2, padx=20, pady=17)

        self.fuel_start_label = tk.CTkLabel(self, text="Enter the US gallons of fuel on board (max 88)",
                                            width=354, font=self.label_font, anchor="w")
        self.fuel_start_label.grid(column=0, row=9, padx=20, pady=17)
        self.fuel_start_entry = tk.CTkEntry(self, width=100, font=self.label_font)
        self.fuel_start_entry.grid(column=1, row=9, padx=20, pady=17)

        self.fuel_taxi_label = tk.CTkLabel(self, text="Enter the planned fuel burned (in US gallons) for taxi",
                                           width=354, font=self.label_font, anchor="w")
        self.fuel_taxi_label.grid(column=0, row=10, padx=20, pady=17)
        self.fuel_taxi_entry = tk.CTkEntry(self, width=100, font=self.label_font)
        self.fuel_taxi_entry.grid(column=1, row=10, padx=20, pady=17)

        self.fuel_flight_label = tk.CTkLabel(self, text="Enter the planned fuel burned (in US gallons) for the flight",
                                             width=354, font=self.label_font, anchor="w")
        self.fuel_flight_label.grid(column=0, row=11, padx=20, pady=17)
        self.fuel_flight_entry = tk.CTkEntry(self, width=100, font=self.label_font)
        self.fuel_flight_entry.grid(column=1, row=11, padx=20, pady=17)
        ...
        self.calculate_button.grid(column=0, columnspan=2, row=12, padx=20, pady=(30, 20))
    ...
    def check_valid(self):
        ...
        self.data['bag3'] = self.bag3_entry.get()
        ...
        if self.data['pax1'] == 0:
            ...
            return
        if (self.data['bag1'] + self.data['bag2'] + self.data['bag3']) > 200:
            CTkMessagebox(title="Error",
                          message="Baggage compartment A + B + C is overweight.",
                          font=("ebrima", 14), fg_color="#353535",
                          button_color="#3EA216")
            return
        if (self.data['bag2'] + self.data['bag3']) > 80:
            CTkMessagebox(title="Error",
                          message="Baggage compartment B + C is overweight.",
                          font=("ebrima", 14), fg_color="#353535",
                          button_color="#3EA216")
            return
        if (self.data['fuel_load']) > 88:
            CTkMessagebox(title="Error",
                          message="Please enter a fuel capacity under the maximum limit for this aircraft.",
                          font=("ebrima", 14), fg_color="#353535",
                          button_color="#3EA216")
            return
        if (self.data['fuel_load'] - (self.data['fuel_taxi'] + self.data['fuel_flight'])) < 7.5:
            CTkMessagebox(title="Error",
                          message="Warning: Less than 6 gallons of fuel will remian after landing. Please recalculate.",
                          font=("ebrima", 14), fg_color="#353535",
                          button_color="#3EA216")
            return
        if (self.data['fuel_load'] - (self.data['fuel_taxi'] + self.data['fuel_flight'])) < 15:
