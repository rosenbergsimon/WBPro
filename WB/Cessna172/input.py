...
class Input(tk.CTkFrame):
    def __init__(self, master, calculate, data):
        ...
        # specific options to the 172's here
        self.ac_options = ["C-GBGV", "C-GLVO", "C-GMRB", "C-GIQB", "C-GMKR", "C-FARC"]
        # sets a StringVar to represent value of the aircraft selected, and to call functions
        self.ac_option = tk.StringVar()  # This code will change the fuel max number on the fuel label (50 53 or 62)
        self.ac_option.set("C-GBGV")  # when the box is clicked to new plane selection
        ...
        self.fuel_start_label = tk.CTkLabel(self, text="Enter the US gallons of fuel on board (max 50)",
                                            width=354, font=self.label_font, anchor="w")
    ...
    def ac_change(self, *args):
        if self.ac_box.get() == "C-GBGV":
            self.fuel_start_label.configure(text="Enter the US gallons of fuel on board (max 50)")
        elif self.ac_box.get() == "C-GLVO":
            self.fuel_start_label.configure(text="Enter the US gallons of fuel on board (max 50)")
        elif self.ac_box.get() == "C-GMRB":
            self.fuel_start_label.configure(text="Enter the US gallons of fuel on board (max 50)")
        elif self.ac_box.get() == "C-GIQB":
            self.fuel_start_label.configure(text="Enter the US gallons of fuel on board (max 50)")
        elif self.ac_box.get() == "C-GMKR":
            self.fuel_start_label.configure(text="Enter the US gallons of fuel on board (max 62)")
        elif self.ac_box.get() == "C-FARC":
            self.fuel_start_label.configure(text="Enter the US gallons of fuel on board (max 53)")
    ...
    def check_valid(self):
        ...
        if (self.data['bag2']) > 50:
            CTkMessagebox(title="Error",
                          ...
                          button_color="#3EA216")
            return
        if (self.data['ident'] == "C-GMKR") and self.data['fuel_load'] > 62:
            CTkMessagebox(title="Error",
                          message="Please enter a fuel capacity under the maximum limit for this aircraft.",
                          font=("ebrima", 14), fg_color="#353535",
                          button_color="#3EA216")
            return
        if (self.data['ident'] == "C-FARC") and self.data['fuel_load'] > 53:
            CTkMessagebox(title="Error",
                          message="Please enter a fuel capacity under the maximum limit for this aircraft.",
                          font=("ebrima", 14), fg_color="#353535",
                          button_color="#3EA216")
            return
        if (self.data['ident'] == "C-GBGV") and self.data['fuel_load'] > 50:
            CTkMessagebox(title="Error",
                          message="Please enter a fuel capacity under the maximum limit for this aircraft.",
                          font=("ebrima", 14), fg_color="#353535",
                          button_color="#3EA216")
            return
        # needs to be repeated due to some bug
        if (self.data['ident'] == "C-GLVO") and self.data['fuel_load'] > 50:
            CTkMessagebox(title="Error",
                          message="Please enter a fuel capacity under the maximum limit for this aircraft.",
                          font=("ebrima", 14), fg_color="#353535",
                          button_color="#3EA216")
            return
        if (self.data['ident'] == "C-GMRB") and self.data['fuel_load'] > 50:
            CTkMessagebox(title="Error",
                          message="Please enter a fuel capacity under the maximum limit for this aircraft.",
                          font=("ebrima", 14), fg_color="#353535",
                          button_color="#3EA216")
            return
        if (self.data['ident'] == "C-GIQB") and self.data['fuel_load'] > 50:
            CTkMessagebox(title="Error",
                          message="Please enter a fuel capacity under the maximum limit for this aircraft.",
                          font=("ebrima", 14), fg_color="#353535",
                          button_color="#3EA216")
            return
        if (self.data['fuel_load'] - (self.data['fuel_taxi'] + self.data['fuel_flight'])) < 5:
            CTkMessagebox(title="Error",
                          message="Warning: Less than 5 gallons of fuel will remian after landing. Please recalculate.",
                          font=("ebrima", 14), fg_color="#353535",
                          button_color="#3EA216")
            return
