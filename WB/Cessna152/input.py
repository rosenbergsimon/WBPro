...
class Input(tk.CTkFrame):
    def __init__(self, master, calculate, data):
        ...
        self.ac_options = ["C-GZHL", "C-GYBI", "C-GOLK", "C-GGKH"]
        ...
        self.ac_option.set("C-GZHL")  # when the box is clicked to new plane selection
        ...
        self.title_label.grid(column=0, columnspan=2, row=1, padx=20, pady=24)

        self.ac_label = tk.CTkLabel(self, text="Select an Airplane from the menu:", font=self.label_font,
                                    width=354, anchor="w")
        self.ac_label.grid(column=0, row=2, padx=20, pady=24)
        ...
        self.ac_box.grid(column=1, row=2, padx=20, pady=24)

        self.front_label = tk.CTkLabel(self, text="Enter the weight (lbs) in the front seats", font=self.label_font,
                                       width=354, anchor="w")
        self.front_label.grid(column=0, row=3, padx=20, pady=24)
        self.front_entry = tk.CTkEntry(self, width=100, font=self.label_font)
        self.front_entry.grid(column=1, row=3, padx=20, pady=24)

        self.bag1_label = tk.CTkLabel(self, text="Enter the weight (lbs) in Baggage Area #1 (max 120)",
                                      width=354, font=self.label_font, anchor="w")
        self.bag1_label.grid(column=0, row=4, padx=20, pady=24)
        self.bag1_entry = tk.CTkEntry(self, width=100, font=self.label_font)
        self.bag1_entry.grid(column=1, row=4, padx=20, pady=24)

        self.bag2_label = tk.CTkLabel(self, text="Enter the weight (lbs) in Baggage Area #2 (max 40)",
                                      width=354, font=self.label_font, anchor="w")
        self.bag2_label.grid(column=0, row=5, padx=20, pady=24)
        self.bag2_entry = tk.CTkEntry(self, width=100, font=self.label_font)
        self.bag2_entry.grid(column=1, row=5, padx=20, pady=24)

        self.bag_note = tk.CTkLabel(self, text="Note: Baggage area 1 and 2 combined limit = 120lbs.",
                                    width=494, font=self.label_font, anchor="w")
        self.bag_note.grid(column=0, row=6, columnspan=2, padx=20, pady=24)

        self.fuel_start_label = tk.CTkLabel(self, text="Enter the US gallons of fuel on board (max 24.5)",
                                            width=354, font=self.label_font, anchor="w")
        self.fuel_start_label.grid(column=0, row=7, padx=20, pady=24)
        self.fuel_start_entry = tk.CTkEntry(self, width=100, font=self.label_font)
        self.fuel_start_entry.grid(column=1, row=7, padx=20, pady=24)

        self.fuel_taxi_label = tk.CTkLabel(self, text="Enter the planned fuel burned (in US gallons) for taxi",
                                           width=354, font=self.label_font, anchor="w")
        self.fuel_taxi_label.grid(column=0, row=8, padx=20, pady=24)
        self.fuel_taxi_entry = tk.CTkEntry(self, width=100, font=self.label_font)
        self.fuel_taxi_entry.grid(column=1, row=8, padx=20, pady=24)

        self.fuel_flight_label = tk.CTkLabel(self, text="Enter the planned fuel burned (in US gallons) for the flight",
                                             width=354, font=self.label_font, anchor="w")
        self.fuel_flight_label.grid(column=0, row=9, padx=20, pady=24)
        self.fuel_flight_entry = tk.CTkEntry(self, width=100, font=self.label_font)
        self.fuel_flight_entry.grid(column=1, row=9, padx=20, pady=24)
        ...
        self.calculate_button.grid(column=0, columnspan=2, row=10, padx=20, pady=(30, 20))

    # part of the StringVar system, changes the fuel capacity label
    def ac_change(self, *args):
        if self.ac_box.get() == "C-GZHL":
            self.fuel_start_label.configure(text="Enter the US gallons of fuel on board (max 24.5)")
        elif self.ac_box.get() == "C-GOLK":
            self.fuel_start_label.configure(text="Enter the US gallons of fuel on board (max 37.5)")
        elif self.ac_box.get() == "C-GYBI":
            self.fuel_start_label.configure(text="Enter the US gallons of fuel on board (max 24.5)")
        elif self.ac_box.get() == "C-GGKH":
            self.fuel_start_label.configure(text="Enter the US gallons of fuel on board (max 37.5)")
    ...
    def check_valid(self):
        ...
        self.data["type"] = "Cessna 152"  # ignore these two
        ...
        if self.data['pax1'] == 0:
            ...
            return
        if (self.data['bag1'] + self.data['bag2']) > 120:
            CTkMessagebox(title="Error",
                          message="Baggage compartment is overweight.",
                          font=("ebrima", 14), fg_color="#353535",
                          button_color="#3EA216")
            return
        if (self.data['bag2']) > 40:
            CTkMessagebox(title="Error",
                          message="Baggage compartment is overweight.",
                          font=("ebrima", 14), fg_color="#353535",
                          button_color="#3EA216")
            return
        if (self.data['ident'] == ("C-GZHL" or "C-GYBI")) and self.data['fuel_load'] > 24.6:
            CTkMessagebox(title="Error",
                          message="Please enter a fuel capacity under the maximum limit for this aircraft.",
                          font=("ebrima", 14), fg_color="#353535",
                          button_color="#3EA216")
            return
        if (self.data['ident'] == ("C-GOLK" or "C-GGKH")) and self.data['fuel_load'] > 37.6:
            CTkMessagebox(title="Error",
                          message="Please enter a fuel capacity under the maximum limit for this aircraft.",
                          font=("ebrima", 14), fg_color="#353535",
                          button_color="#3EA216")
            return
        if (self.data['fuel_load'] - (self.data['fuel_taxi'] + self.data['fuel_flight'])) < 3:
            CTkMessagebox(title="Error",
                          message="Warning: Less than 3 gallons of fuel will remian after landing. Please recalculate.",
                          font=("ebrima", 14), fg_color="#353535",
                          button_color="#3EA216")
            return
        if (self.data['fuel_load'] - (self.data['fuel_taxi'] + self.data['fuel_flight'])) < 7:
